...
public class Renderer extends JPanel implements DomainObserver {
  ...
  public Renderer(Domain domain) {
    ...
    GRID_SIZE = domain.getTiles().length;
    setPreferredSize(new Dimension(FOCUS_SIZE * 50, FOCUS_SIZE * 50));
  }
  ...
  protected void paintComponent(Graphics g) {
    ...
    int cellHeight = getHeight() / FOCUS_SIZE;
    ...
    for (int row = focusTopRow; row < focusTopRow + FOCUS_SIZE && row < GRID_SIZE; row++) {
      for (int col = focusLeftCol; col < focusLeftCol + FOCUS_SIZE && col < GRID_SIZE; col++) {
    ...
    if (domain.getFailed()) {
      //draw black screen over the game and display game over message
